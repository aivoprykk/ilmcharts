...
[ x"$place" = x"" ] && continue;
if [ x"$mnt" != x"" ]; then
mnt_url='http://www.balticroads.net/?getstationdata=1&mapstation='$mnt
mnt_out="mnt_data/"$place
mnt_temp="arc-file-$place.html"
mnt_file="ARC-"$date".txt"

[ -d "$mnt_out" ] || mkdir -p "$mnt_out"

if [ -f $mnt_out/$mnt_file ]; then
last_stamp=`cat $mnt_out/$mnt_file|tail -1|awk '{ sub("([0-9][0-9][0-9][0-9])","&-",$1);sub("-([0-9][0-9])","&-"); print " " $1 " " $2 }'`
...
if [ $lab -gt 0 ]; then
  echo "wget -q -O '$mnt_out/$mnt_temp' '$mnt_url' at `date '+%F %T'`"
fi
if [ x"$dry" = x"" ]; then
  wget -q -O "$mnt_out/$mnt_temp" "$mnt_url"
  node $dir"/parse_mnt.js" $mnt_out/$mnt_temp
  #rm -f $mnt_out"/"$mnt_temp
fi
fi
)
done
